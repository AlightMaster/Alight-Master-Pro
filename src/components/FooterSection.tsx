...
export const FooterSection: React.FC<FooterSectionProps> = ({ onNavigate, onOpenAdmin }) => {
  ...
  const loadLinks = () => {
    setSocialTg(localStorage.getItem('alight_link_social_tg') || 'https://t.me');
    setSocialTiktok(localStorage.getItem('alight_link_social_tiktok') || 'https://tiktok.com');
    setSocialIg(localStorage.getItem('alight_link_social_ig') || 'https://instagram.com');
    setSocialHandle(localStorage.getItem('alight_social_handle') || '@ALIGHTMASTER');
  };
    ...
    <footer className="mt-12 bg-white dark:bg-slate-900 border-t-2 border-slate-900 dark:border-slate-600 pt-8 pb-20 px-4 select-none">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="space-y-3">
          ...
          >
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 border-2 border-slate-900 dark:border-slate-600 flex items-center justify-center text-white font-black text-lg shadow-[2px_2px_0px_#0f172a] dark:shadow-[2px_2px_0px_#475569] group-hover:rotate-3 transition-transform">
              A
            </div>
            <div>
